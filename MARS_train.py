...
if __name__=="__main__":
    ...
    opt = parse_opts()
    print("#"*50)
    for op in ['MARS', 'MARS_pretrain_path', 'MARS_resume_path', 'dataset', 'ft_begin_index', 'modality', 'model', 'model_depth', 'n_classes', 'n_finetune_classes', 'only_RGB']:
        print("{}: {}".format(op, vars(opt)[op]))
    print("-"*50)
    for op in ['batch_size', 'begin_epoch', 'dampening', 'learning_rate', 'lr_patience', 'weight_decay']:
        print("{}: {}".format(op, vars(opt)[op]))
    print("#"*50)
    
    opt.arch = '{}-{}'.format(opt.model, opt.model_depth) # resnext-101
    torch.manual_seed(opt.manual_seed)

    print("Preprocessing train data ...")
    train_data = globals()['{}_test'.format(opt.dataset)](split = opt.split, train = 1, opt = opt)
    print("Length of train data = {} ({})".format(len(train_data), train_data.data))

    print("Preprocessing validation data ...")
    val_data = globals()['{}_test'.format(opt.dataset)](split = opt.split, train = 2, opt = opt)
    print("Length of validation data = {} ({})".format(len(val_data), val_data.data))
    ...
    print("Length of train datatloader: [{}] as Number of train data is [{}] ".format(len(train_dataloader), len(train_data)))
    print("Length of validation datatloader: [{}] as Number of validation data is [{}] ".format(len(val_dataloader), len(val_data)))
    ...
    #########################
    # Hyper-parameters depends on datasets
    #########################
    ...
    #########################
    print("#"*50)
    print("Loading MARS model... ", opt.model, opt.model_depth) # e.g) resnext 101 depth
    ...
    if opt.pretrain_path != '':
        ...
        if opt.dataset == 'HMDB51':
            ...
            opt.n_classes = 101 
        elif opt.dataset == 'Fight':
            opt.n_classes = 2
        elif opt.dataset == 'Speaking':
            opt.n_classes = 2
        elif opt.dataset == 'MOBIO':
            opt.n_classes = 2
    ...
    # Define Loss Func
    #########################
    criterion_MARS  = nn.CrossEntropyLoss().to(device)
    criterion_Flow = nn.MSELoss().to(device)
    ...
    if opt.MARS_resume_path:
        ...
        checkpoint = torch.load(opt.MARS_resume_path, map_location=load_device)
    ...
    scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience=opt.lr_patience, factor=0.2)
